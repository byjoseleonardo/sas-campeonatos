...
function supervisorFullName(s: Supervisor) {
  return [s.firstName, s.paternalLastName, s.maternalLastName].filter(Boolean).join(" ");
}
...
export default function AdminSupervisoresPage() {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      toast({ title: "Supervisor creado", description: `${form.firstName} ${form.paternalLastName} fue registrado exitosamente.` });
      setCreateOpen(false);
      setForm(emptyForm);
      fetchSupervisores();
    } catch (e: unknown) {
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/users/${editSupervisor.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error);
      toast({ title: "Supervisor actualizado", description: "Los cambios fueron guardados." });
      setEditOpen(false);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast({ title: "Supervisor eliminado", description: "El supervisor fue removido del sistema." });
      fetchSupervisores();
    } catch {
      toast({ title: "Error", description: "No se pudo eliminar el supervisor", variant: "destructive" });
    } finally {
      setDeleteId(null);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="font-display text-3xl sm:text-4xl text-foreground">SUPERVISORES</h1>
          <p className="text-muted-foreground text-sm mt-1">Crea y gestiona los supervisores de tus campeonatos</p>
        </div>

        {/* CREATE DIALOG */}
        <Dialog open={createOpen} onOpenChange={(v) => { setCreateOpen(v); if (!v) setForm(emptyForm); }}>
          <DialogTrigger asChild>
            <Button><Plus className="h-4 w-4 mr-1" /> Nuevo Supervisor</Button>
          </DialogTrigger>
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              <DialogTitle>Crear Supervisor</DialogTitle>
              <DialogDescription>Registra un supervisor y asígnalo a un campeonato.</DialogDescription>
            </DialogHeader>
            <form onSubmit={(e) => { e.preventDefault(); handleCreate(); }} className="space-y-4 py-2">
              ...
              <div className="space-y-2">
                ...
                <Input type="email" placeholder="supervisor@email.com" autoComplete="new-email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} />
              </div>
              <div className="space-y-2">
      ...
      {/* EDIT DIALOG */}
      <Dialog open={editOpen} onOpenChange={(v) => { setEditOpen(v); if (!v) { setEditSupervisor(null); setForm(emptyForm); } }}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>Editar Supervisor</DialogTitle>
            <DialogDescription>Modifica los datos del supervisor.</DialogDescription>
          </DialogHeader>
          <form onSubmit={(e) => { e.preventDefault(); handleEdit(); }} className="space-y-4 py-2">
            ...
            <DialogFooter>
              <Button type="button" variant="outline" onClick={() => { setEditOpen(false); setEditSupervisor(null); setForm(emptyForm); }}>Cancelar</Button>
              <Button type="submit" disabled={saving}>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Esta acción eliminará al supervisor y todos sus registros asociados. No se puede deshacer.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
        ...
        <CardContent>
                    ...
                    <TableRow key={s.id}>
                      <TableCell>
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="text-sm font-medium">{supervisorFullName(s)}</p>
                      ...
                      <TableCell>
                        <span className={`inline-block rounded-full px-2.5 py-0.5 text-xs font-medium ${
                          s.isActive ? "bg-primary/15 text-primary" : "bg-muted text-muted-foreground"
                        }`}>
          ...
          {!loading && supervisores.length === 0 && (
            <p className="text-center py-8 text-muted-foreground text-sm">
              No hay supervisores registrados. Crea el primero con el botón de arriba.
            </p>
          )}
        </CardContent>
      </Card>
