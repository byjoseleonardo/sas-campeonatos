...
export default function AdminTecnicosPage() {
  ...
  const tecnicos = adminUsers.filter((u) => u.role === "tecnico");
  ...
  const handleCreate = () => {
    toast({
      ...
      description: "El técnico se ha asignado exitosamente (demo).",
    });
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="font-display text-4xl text-foreground">TÉCNICOS</h1>
          <p className="text-muted-foreground text-sm mt-1">
            Gestiona los técnicos que administran las estadísticas de juegos
          </p>
        </div>
        <Dialog>
          <DialogTrigger asChild>
            <Button>
              <Plus className="h-4 w-4 mr-1" /> Asignar Técnico
            </Button>
          </DialogTrigger>
