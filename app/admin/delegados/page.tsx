...
export default function AdminDelegadosPage() {
  ...
  const delegados = adminUsers.filter((u) => u.role === "delegado");
  const filtered = delegados.filter(
    (u) =>
  ...
  const handleCreate = () => {
    toast({
      title: "Delegado creado",
      description: "El delegado se ha registrado exitosamente (demo).",
    });
  };
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            Gestiona los delegados asignados a cada equipo
          </p>
        </div>
        <Dialog>
          <DialogTrigger asChild>
            <Button>
              <Plus className="h-4 w-4 mr-1" /> Nuevo Delegado
            </Button>
          </DialogTrigger>
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              <DialogTitle>Registrar Delegado</DialogTitle>
            </DialogHeader>
